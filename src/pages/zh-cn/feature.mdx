...
... 的 404 页面由两个文件组成。如果要自定义 404 页面的设计，通常只需编辑 `src/pages/[lang]/404.astro` 文件即可。
...
当用户访问 ... 并访问此文件时，根据用户的系统语言等，将重定向到以下 404 页面。
...
因此，请注意在...的`src/styles/base.css`中定义了以下内容：
...
在...中，我们使用[Material Icon](https://fonts.google.com/icons)，在`src/styles/base.css`中定义了以下内容：
