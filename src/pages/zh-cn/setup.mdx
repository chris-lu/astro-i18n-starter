...
不建议更改 `prefixDefaultLocale` 和 `redirectToDefaultLocale` 的设置。... 使用客户端 JavaScript 管理重定向过程。如果 URL 不包含语言环境，则会重定向到默认语言环境。例如，`/setup/` 将重定向到 `/en/setup/`。
...
包含在 ... 中使用的函数的定义的文件。通常情况下，不需要编辑此文件。
