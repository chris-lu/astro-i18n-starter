...
... の404ページは2つのファイルから構成されています。404ページのデザインをカスタムしたい場合、基本的に `src/pages/[lang]/404.astro` だけを編集すればOKです。
...
... でこのファイルにアクセスされた場合、ユーザーのシステム言語などに対応して、以下の404ページにリダイレクトします。
...
そのため...では、`src/styles/base.css`の中に以下が定義されていることに注意してください。
...
...では[Material Icon](https://fonts.google.com/icons)を使用しており、`src/styles/base.css`の中に以下が定義されています。
