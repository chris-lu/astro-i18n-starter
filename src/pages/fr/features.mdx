...
The { "site.title" } 404 page consists of two files. If you want to customize the design of the 404 page, you generally only need to edit `src/pages/[lang]/404.astro`.
...
When accessed with { "site.title" }, it redirects to the following 404 page based on the user's system language:
...
Therefore, please note that { "site.title" } defines the following in `src/styles/base.css`:
...
{ "site.title" } uses [Material Icon](https://fonts.google.com/icons), and the following is defined in `src/styles/base.css`:
