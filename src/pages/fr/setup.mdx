...
Changing the settings of `prefixDefaultLocale` and `redirectToDefaultLocale` is not recommended. If the URL does not contain a locale, it will redirect to the default locale. For example, `/setup/` will be redirected to `/en/setup/`.


### 2. Configure /src/locales.ts

Similarly, update the configuration file.
...
File containing definitions of functions used in { "site.title" }. There is generally no need to edit this file.
