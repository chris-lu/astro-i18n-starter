...
لا يُنصح بتغيير إعدادات `prefixDefaultLocale` و `redirectToDefaultLocale`. ... يدير عملية إعادة التوجيه باستخدام جافا سكريبت على الجانب العميل. إذا لم يحتوي عنوان URL على لغة، فسيتم إعادة التوجيه إلى اللغة الافتراضية. على سبيل المثال، سيتم إعادة التوجيه من `/setup/` إلى `/en/setup/`.


### 2. تكوين /src/locales.ts

بالمثل، قم بتحديث ملف التكوين لـ ....
...
ملف يحتوي على تعريفات الوظائف المستخدمة في .... عمومًا، لا يوجد حاجة لتحرير هذا الملف.
